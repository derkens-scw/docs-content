...
meta:
  ...
  description: This page show an example of serverless architecture for asynchronous file conversion based on messages, functions, and triggers.
content:
  ...
  paragraph: This page show an example of serverless architecture for asynchronous file conversion based on messages, functions, and triggers.
categories:
  - messaging
  - functions
  - object-storage
tags: message nats serverless serverless-functions functions serverless-triggers triggers storage object-storage terraform large-messages big-data
dates:
...
In this tutorial, we will build a Cloud architecture to store and automatically convert images to PDF. The focus is on transferring large messages using a messaging service, specifically the [Scaleway Messaging and Queuing](/serverless/messaging/quickstart/) NATS server. Our setup involves using the [Object Storage](/storage/object/quickstart) service for image storage and the [Serverless Functions](/serverless/functions/quickstart/) service for conversion.
...
<Message type="requirement">
  ...
  - You have [Terraform](/developer-tools/terraform/quickstart/) installed on your local machine
  ...
  - You have [Docker](/tutorials/install-docker-ubuntu-bionic/) installed on your local machine
  - You have [NATS CLI](https://github.com/nats-io/natscli) installed on your local machine
  - You have [AWS CLI configured with Scaleway credentials](https://www.scaleway.com/en/docs/storage/object/api-cli/object-storage-aws-cli/#how-to-install-the-aws-cli)
</Message>
...
This tutorial's architecture consists of two main components:
...
As mentioned earlier, the producer will be implemented as a shell script.
...
7. Add the resource to set up the function.
    ...
    ```
    The code sets up the function and connects it to the function namespace. It tells the system to use Python version 3.11 and specifies which part of the Python file should run (the handler). It also adds important information, like environment variables and secrets, that our function will need to work correctly.
